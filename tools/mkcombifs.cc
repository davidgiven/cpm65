...
{
	std::fstream fs(argv[1], std::ios::binary | std::ios::in | std::ios::out);
	if (!fs)
		error("Cannot open input file: {}", strerror(errno));
	uint32_t size = std::filesystem::file_size(argv[1]);
	...
	fmt::print("1541 filesystem has {} allocated sectors\n", usedSectors.size());

	std::set<int> usedBlocks;
	for (int sector : usedSectors)
		usedBlocks.insert(sector/4);

	fmt::print("CP/M filesystem has {} allocated blocks\n", usedBlocks.size());
