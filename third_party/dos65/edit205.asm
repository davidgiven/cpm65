...
.zp pzstrt, 0                           \start of zero page
.zp prmsgn, 1                           \parameter sign (0=-,ff=+)
.zp number, 2                           \parameter value
.zp column, 1                           \console column
.zp curdrv, 1                           \current drive (default)
.zp dstdrv, 1                           \destination drive
.zp nxtchr, 1                           \next character from input
.zp cnsind, 1                           \index into cnstxt
.zp macflg, 1                           \macro flag and maximum index
.zp macind, 1                           \macro buffer index
.zp insflg, 1                           \insert mode if >127
.zp nomore, 1                           \no more console input if >127
.zp libind, 1                           \.lib file index
.zp xlbind, 1                           \x$$$$$$$.lib file index
.zp rdsccn, 1                           \read sector count
.zp strind, 1                           \index into strbuf
.zp nxttxt, 2                           \pointer to next char position
.zp srcind, 2                           \source file pointer
.zp dstind, 2                           \destination file pointer
.zp uprtxt, 2                           \upper text pointer value
.zp lwrlmt, 2                           \lower move limit
.zp uprlmt, 2                           \maximum text pointer
.zp maxtxt, 2                           \txtlmt-1
.zp txtlmt, 2                           \limit of text buffer
.zp point, 2                            \general use buffer
.zp maccnt, 2                           \macro count
.zp typpnt, 2                           \pointer for type
.zp cmppnt, 2                           \pointer for compre
.zp prsstr, 1                           \parse start
.zp endstr, 1                           \end of search string
.zp xlbpnt, 2                           \x$$$$$$$.lib pointer
.zp normal, 1                           \normal video
.zp invert, 1                           \invert video
.zp forwar, 1                           \forward space
.zp clreol, 1                           \clear to eol
.zp backsp, 1                           \backspace
.zp endcol, 1                           \last column
.zp outflg, 1                           \output ok if < 128
...
tstblk: lda     #blank          \test for blank
        cmp     dflfcb+1        \test first char
        beq     nmeerr          \error if blank
        cmp     dflfcb+17       \then if second
...
rentry: lda     #<source_end    \set
        ldy     #>source_end    \source
        sta     srcind          \index to
        sty     srcind+1        \illegal
        lda     #<dest          \set
        ldy     #>dest          \destination
        sta     dstind          \pointer
        sty     dstind+1        \to first
        lda     #0              \clear
...
dltok:  jsr     getcol          \set current column
        ...
        cmp     #lf             \if a lf
        beq     dlt             \then kill cr too
        cmp     #cr             \if a cr
        beq     dorpt           \type new line
...
ntmxnu: jsr     tstdec          \test for decimal
        bcc     gotnum          \if not jump
        jsr     bldnum          \else build parm
gotnum: jsr     tstnm0          \test for zero
        bne     *+6             \branch if not
...
appnlp: jsr     tstadj          \test for zero
...
fndstr: jsr     tstadj          \test number
        bne     *+5             \continue if not zero
        jmp     prsmre          \else done
        jsr     match           \try to match
...
maclpe: inc     macind          \bump index
        ...
        ldy     number+1        \macro
        sta     maccnt          \count
        sty     maccnt+1        \to number
        jmp     prsmre          \and loop
\s for substitute strings
notmac: cmp     #'S'            \if not s
        bne     notsub          \try next
        jsr     clstfl          \get search string
        jsr     flstbf          \then replacement
sublpe: jsr     tstadj          \test number
        bne     *+5             \jump if more
        jmp     prsmre          \else done
        jsr     match           \try to match
        sec                     \drop
...
getcol: lda     #$ff            \turn off output
        sta     outflg
        jsr     clear           \do a cr
        jsr     stnm0           \type line to get column
        lda     #minus
        sta     prmsgn
        jsr     type
...
lbinok: inc     libind          \bump fo next
        ...
        rts
\test for number = zero (z=1 if zero)
...
stnmmx: lda     #$ff            \set
        ...
        sta     number+1        \to $ff
        rts
\test for zero and if not drop number (z=1 if zero)
tstadj: jsr     tstnm0          \test for zero
        bne     *+3             \jump if not
        rts
...
dflprm: lda     #plus           \set sign
        ...
        sty     number+1        \1
        rts
\test uprtxt against uprlmt (c=0 if uprtxt < uprlmt)
tsupup: lda     uprtxt          \compare upper
        cmp     uprlmt          \to upper limit
        lda     uprtxt+1        \and set
        sbc     uprlmt+1        \carry
        rts                     \accordingly
\test nxttxt against lwrlmt (c=0 if nxttxt > lwrlmt)
tsnxlw: lda     lwrlmt          \compare next
        cmp     nxttxt          \to lower limit
        lda     lwrlmt+1        \and set
        sbc     nxttxt+1        \carry
        rts                     \accordingly
\get next byte from source
getsrc: lda     srcind          \compare
        cmp     #<source_end    \pointer
        lda     srcind+1        \to
        sbc     #>source_end    \maximum
        bcc     *+5             \use if less
...
movdpl: jsr     tsupup          \test upper against upper limit
        bcc     *+3             \continue if less
        rts                     \else done
        jsr     bpuptx          \bump upper pointer
        jmp     movdpl          \and loop
...
rdesok: clc                     \add
        ...
        jmp     clsind          \then set index to start
...
movnme: ldx     #15             \move all
        ...
        sta     dstfcb+16,x     \to second
        dex                     \half
        bpl     movnme+2        \then
        rts                     \exit
\test char for first and only command and not macro
\ z=1 if true
...
tstfex: rts
\test for first and only and verify
tstvrf: pha                     \save char
        jsr     tstfon          \test for first
        ...
        cmp     #'Y'            \see if y
        rts
\test for decimal (if decimal then c=1 else c=0)
...
cnvlwr: jsr     tstlwr          \test for lower
        bcc     *+4             \exit if not
        and     #0b01011111     \else convert
        rts
\get input character
getchr: lda     macflg          \get flag
        beq     notmci          \branch if not macro
        jsr     consts          \test for break
...
usmcin: inc     macind          \bump for next
        ...
        rts
\not macro
notmci: bit     insflg          \test insert mode
        bpl     ntinmd          \jump if not
        jmp     cnsin           \get char
\command mode
ntinmd: bit     nomore          \test for no input
...
write:  lda     #minus          \set sign
        sta     prmsgn          \to minus
        lda     #<txtbuf+1      \then set
        ldy     #>txtbuf+1      \pointer
        sta     lwrlmt          \for
        sty     lwrlmt+1        \move
        lda     uprtxt          \to beginning
        ldy     uprtxt+1        \of the
        sta     uprlmt          \text
        sty     uprlmt+1        \buffer
